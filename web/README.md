...
| `VITE_GRAFANA_PANELS` | `1:Shooting ratio by scene,2:Pages vs plan,3:Flag rate,4:Camera hours` | Comma-separated `panelId:Title` list. |
...
- `GET /api/takes?scene=12` → `[{take_id, scene_number, shot, take_number, status, clip_uri,
  thumb_uri, duration_s, quality_score, flags[], summary}]`. `scene` omitted ⇒ all takes.
- `GET /api/take/<id>/events` → `[{t, kind:"dialogue"|"action"|"flag", speaker?, text?, flag?}]`.
  **Optional** — a 404 just hides the transcript timeline.
...
- `GET /clips/<file>.mp4`, `/clips/<file>.jpg`.
...
- The **final answer text may end with a fenced ```json block** of the form
  `{"takes":[{take_id, clip_uri, thumb_uri, scene_number, shot, take_number, status, t?}], "sql":[…]}`.
  The UI parses it out of the visible markdown and renders players + a SQL disclosure. Any other
  fenced JSON is left in the prose untouched.
