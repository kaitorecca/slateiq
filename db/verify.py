...
def main() -> int:
    ...
    for mv, src in (("take_daily_agg", "day_number"), ("take_scene_agg", "scene_number")):
        try:
    ...
    checks = [
        ("days 13-30 have no takes",
         "SELECT count() FROM slateiq.take WHERE day_number > 12", lambda v: v == 0),
        ("day 12 is the latest shot day",
         "SELECT max(day_number) FROM slateiq.take", lambda v: v == 12),
        ("30 shooting days planned",
         "SELECT count() FROM slateiq.shooting_day", lambda v: v == 30),
        ...
        ("day 11 behind schedule",
         ...
         lambda v: v == 1),
        ("overtime exists",
         "SELECT count() FROM slateiq.daily_progress WHERE wrap_delay_min > 0", lambda v: v >= 3),
        ...
        ("every take has an analysis row",
         ...
         "USING (production_id,take_id) WHERE a.take_id = ''", lambda v: v == 0),
        ("all 8 flag types present",
         "SELECT uniqExact(flag_type) FROM slateiq.take_event WHERE kind='flag'", lambda v: v == 8),
        ...
        ("telemetry joins to takes",
         "SELECT uniqExact(take_id) FROM slateiq.frame_telemetry", lambda v: v > 1000),
        ("soft-focus telemetry correlates with flags",
         "SELECT countIf(focus_score < 0.55) FROM slateiq.frame_telemetry", lambda v: v > 1000),
        ("reserved ingest scenes exist in scene table",
         "SELECT count() FROM slateiq.scene WHERE scene_number IN "
         "('12','14A','27','33','41','56','78','102')", lambda v: v == 8),
        ("scene_progress covers all 120 scenes",
         "SELECT count() FROM slateiq.scene_progress", lambda v: v == 120),
        ("flag_summary populated",
         "SELECT count() FROM slateiq.flag_summary", lambda v: v > 20),
    ]
    ...
    for label, sql in golden_queries(os.path.join(HERE, "SCHEMA.md")):
        try:
            ...
            check(f"golden Q{label[:44]}", len(res.result_rows) > 0,
                  f"{len(res.result_rows)} rows")
        except Exception as exc:                                # noqa: BLE001
