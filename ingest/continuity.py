...
def notes_for_scene(cl, scene: str, live: bool = True) -> list[dict]:
    ...
    resp, used = generate(cl, contents=prompt, config={
    ...
    valid = [n for n in data["notes"]
             if n["take_id_a"] in ids and n["take_id_b"] in ids
             and n["take_id_a"] != n["take_id_b"]]
...
def main() -> int:
    ...
    for scene, n in rows:
        print(f"  {scene:>4}  {n['category']:<14} s{n['severity']}  "
