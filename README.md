...
| **Google Cloud Agent Builder (Agent Development Kit / ADK)** — multi-agent | ADK 2.8. `slateiq_coordinator` (`LlmAgent`) routes via `transfer_to_agent` to **4 specialists**: `editor_agent`, `production_agent`, `continuity_agent`, `report_agent`. | [`agent/slateiq_agent/agent.py:47-52`](agent/slateiq_agent/agent.py) (`SUB_AGENT_NAMES`), `:213-254` (builders) · `google-adk` in [`agent/requirements.txt`](agent/requirements.txt) |
| **ClickHouse via the OFFICIAL `mcp-clickhouse`, at runtime** | Every analytical answer is SQL the agent wrote and executed through one shared ADK `McpToolset` over StreamableHTTP. The specialists have **no other data tool** — there is no database driver on the reasoning path. | [`agent/slateiq_agent/agent.py:162-187`](agent/slateiq_agent/agent.py) (`build_clickhouse_toolset` → `StreamableHTTPConnectionParams`), `:194-211` (`tools=[toolset]`) · [`config.py:31-35`](agent/slateiq_agent/config.py) (`CLICKHOUSE_MCP_URL`) · [`runtime.py:231`](agent/slateiq_agent/runtime.py) (`run_query` SQL captured for the trace) · [`guardrails.py:313`](agent/slateiq_agent/guardrails.py) (`if tool.name != "run_query"`) · `mcp-clickhouse==0.6.0` in [`deploy/vm/compose/`](deploy/vm/compose) |
